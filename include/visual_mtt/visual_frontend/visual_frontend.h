  ...
  */
  ...
  private:
    ...
    */
    ...
    // ??????????????????????????????????????????
    uint32_t CallbackElevationEvent(double x, double y);
    ...
    */
    void PublishTracks(const std::vector<rransac::core::ModelPtr>& tracks);
    ...
    * @param tracks Tracks generated by R-RANSAC.
    */
    cv::Mat DrawTracks(const std::vector<rransac::core::ModelPtr>& tracks);
