...
DifferenceImage::DifferenceImage()
{
  ...
  source_parameters_.meas_cov_ = Eigen::Matrix<double,2,2>::Identity()*pow(sigmaR_pos_,2);
...
void DifferenceImage::SetParameters(const visual_mtt::visual_frontendConfig& config)
{
  ...
  if (source_parameters_.gate_threshold_!=config.RRANSAC_gate_threshold || sigmaR_pos_ != config.difference_image_sigmaR_pos) {
      sigmaR_pos_ = config.difference_image_sigmaR_pos;
      source_parameters_.gate_threshold_ = config.RRANSAC_gate_threshold;
      source_parameters_.meas_cov_ = Eigen::Matrix2d::Identity()*sigmaR_pos_;
